...
def column_filter(dataframe, required_column_names):
    ...
    dataframe.drop(
        columns=[
            column for column in all_columns if column not in required_column_names
        ]
    )
...
def date_formatter(dataframe, col_id, date_format, runtime_params):
    ...
    dataframe[col_id] = dataframe[col_id].map(
        lambda x: x.strftime(final_date_format) if pd.notnull(x) else ""
    )
    return dataframe
...
def constant_date_formatter(dataframe, col, format_options, runtime_params):
    try:
        ...
        error_msg = (
            "Missing required formatting attributes for constant date formatter. It should be a "
            'list of date_offset, date_format and calendar_country. e.g., [0, "%Y%m%d", "US"]'
        )
        log.error(error_msg)
        raise

    dataframe[col] = get_business_day(
        datetime.today() + timedelta(date_offset), "next", calendar_country
...
def duplicate_column_formatter(
    dataframe, duplicate_col_name, original_col_name, runtime_params
):
    """
...
def group_percentage_formatter(dataframe, col_name, format_options, runtime_params):
    ...
    dataframe[col_name] = dataframe.groupby(in_col)[of_col].transform(
        lambda x: 100 * x / sum(x)
    )
    return dataframe
...
def date_diff_formatter(dataframe, col_name, format_options, runtime_params):
    ...
    if from_date not in dataframe or to_date not in dataframe:
        raise KeyError(
            "Column does not exist in dataframe. Check column names in the date-diff formatter config"
        )
    delta = pd.to_datetime(dataframe[from_date], format=date_format) - pd.to_datetime(
        dataframe[to_date], format=date_format
    )
...
def bucket_formatter(dataframe, col_name, config, runtime_params):
    ...
    include_right = (
        True if str.lower(config.get("include_right", "true")) == "true" else False
    )

    categories = pd.cut(
        dataframe[col_name], bins=bins, labels=labels, right=include_right
    )
    dataframe[col_name] = categories.values.astype(str)
    return dataframe


def arithmetic_calculation_formatter(
    dataframe, col_name, format_options, runtime_params
):
    """
    ...
    if operation not in valid_operations:
        log.error(
            f"Unknown arithmetic operation. Valid operations are {valid_operations}"
        )
        raise ValueError
...
def fill_empty_values_with_custom_value(
    dataframe, col_name, format_options, runtime_params
):
    """
    ...
    if condition is not None and len(condition) == 2:
        dataframe[col_name] = np.where(
            np.logical_and(
                ...
                (
                    dataframe[condition.get("match_col")].str.match(
...
def split_column_formatter(dataframe, col_name, format_options, runtime_params):
    ...
    if dataframe[col_name].dtype == object and isinstance(
        dataframe.iloc[0][col_name], str
    ):
        ...
        split_df = pd.DataFrame(
            dataframe[col_name].tolist(), columns=format_options.get("new_col_names")
        )
...
def conditional_replace_formatter(dataframe, col_name, format_options, runtime_params):
    ...
    dataframe[col_name] = np.where(
        match_col.str.match(pat=pattern), dataframe[from_column], dataframe[col_name]
    )
    return dataframe
...
def replace_value(dataframe, col_name, format_options, runtime_params):
    ...
    for from_value, to_value in zip(
        format_options.get("from_value"), format_options.get("to_value")
    ):
        dataframe[col_name] = dataframe[col_name].replace({from_value: to_value})
    return dataframe
...
def extract_from_pattern(dataframe, col_name, format_options, runtime_params):
    ...
    dataframe[col_name] = dataframe[col_name].map(
        lambda x: None
        if len(re.findall(pattern, str(x))) == 0
        else (re.findall(pattern, str(x)))[0]
    )
    return dataframe
...
def runtime_date(dataframe, col_name, format_options, runtime_params):
    ...
    return date_formatter(
        dataframe, col_name, {"src": "%m%d%Y", "des": format_options.get("des")}, {}
    )
...
def float_precision(dataframe, col_name, format_options, runtime_params):
    ...
    dataframe[col_name] = dataframe[col_name].map(
        lambda x: float(f"%.{precision}f" % x)
    )
    return dataframe
