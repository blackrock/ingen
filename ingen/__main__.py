...
def main(config_path, query_params, run_date, interfaces, infile=None, dynamic_data=None):
    ...
    for metadata in metadata_list:
        try:
            ...
            logger.info(f"Successfully generated interface '{metadata.name}' in {end - start:.2f} seconds.")
        except Exception as e:
            logger.error(f"Failed to generate interface file for {metadata.name} \n {e}")
    main_end = time.time()
    logger.info(f"Interface Generation finished. Time taken: {main_end - main_start:.2f} seconds")
...
if __name__ == '__main__':
    ...
    if args.infile:
        logger.info(f"Use poller $infile when source use_infile is turned on, overwrite source file_path: {args.infile}")
