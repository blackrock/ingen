<!doctype html><html><head><meta charset="utf-8"><title>{% block title %}{{page_title | e}}{% endblock %}</title>{% block favicon %}<link rel="shortcut icon" type="image/x-icon" href="/static/favicons/favicon.ico">{% endblock %}<script defer="defer" src="{{page_config.fullStaticUrl}}/main.52db296c105d9e7a2830.js?v=52db296c105d9e7a2830"></script></head><body class="jp-ThemedContainer">{% block stylesheet %}<style>/* disable initial hide */
div#header, div#site {
    display: block;
